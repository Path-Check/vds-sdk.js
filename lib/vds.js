...
function sigDERtoRAW(signatureDER) {
  const obj = ASN.ASN1.fromDER(signatureDER, true);
  const captures = {};
  const err = obj.validate(SignatureValidator, captures);

  return Buffer.concat([captures.r.bytes, captures.s.bytes]);
}

function sigRAWtoDER(signatureRaw) {
...
export async function sign(payload, certificate, privateKeyP8) {
  ...
  clonePayload["sig"]["sigvl"] = base64toBase64URL(sigDERtoRAW(signatureDER).toString('base64')); 
...
export async function verify(payload) {
  ...
  try {
    ...
    if (CERT.publicKey.verify(canonicalize(clonePayload.data), sigRAWtoDER(signatureRaw), 'sha256')) {
      return clonePayload;
    }
  } catch (err) {
    console.error(err);
